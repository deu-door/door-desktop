...
export const FetchButton: React.FC<
	FetchButtonProps & React.HTMLAttributes<HTMLDivElement>
> = props => {
	...
	const onVisibilityChange = (isVisible: boolean) => {
		if (action && isVisible && !pending) {
			dispatch(
				fulfilled
					? action.fetchIfExpired()
					: action.fetchIfNotFulfilled(),
			);
		}
	};

	const innerComponent = error ? (
		'에러 : ' + error
	) : fulfilled ? (
		<span>
			새로고침: <DateTime date={fetchedAt} relative />
		</span>
	) : (
		'불러오기'
	);
