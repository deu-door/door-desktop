			...
			<SnackbarContent
				...
				message={<>
					<Grid container justify="space-between" alignItems="center" spacing={1}>
						<Grid item>
							{!item.complete ? `${item.name} 파일을 다운로드 중입니다 ...` : `${item.name} 파일이 다운로드되었습니다.`}
						</Grid>
						<Grid item>
					...
					{!item.complete && <>
						...
						<div><LinearProgress variant="determinate" value={item.progress} className={classes.notificationProgress} /></div>
					</>}
				</>}
			/>
		</Snackbar>
...
export const Downloads: React.FC = props => {
	...
	useEffect(() => {
		const update = (item: Electron.DownloadItem, properties: { message?: string, complete: boolean }) => {
		...
		const onProgress = (item: Electron.DownloadItem) => update(item, { complete: false });

		const onComplete = (item: Electron.DownloadItem) => update(item, { complete: true });
