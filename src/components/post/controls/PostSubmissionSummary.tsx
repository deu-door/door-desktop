...
export const PostSubmissionSummary: React.FC<PostSubmissionSummaryProps> = props => {
	...
	const progress = ((new Date().valueOf() - new Date(from).valueOf()) / duration) * 100;
	...
	return (
		notStarted ? 
			<Alert severity="info">
				<AlertTitle>
					<DateTime date={from} relative precision="seconds" /> 제출할 수 있습니다.
				</AlertTitle>
		...
		:
			<Alert severity="info">
				<AlertTitle>
					<DateTime date={to} relative precision="seconds" /> 제출이 마감됩니다.
				</AlertTitle>
				제출기한: <DateTime date={from} /> ~ <DateTime date={to} />
			</Alert>
	);
