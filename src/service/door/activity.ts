...
export async function getActivity(courseId: ID, id: ID): Promise<Activity> {
	...
	if (!descriptionTable || !submissionTable || !form)
		throw new Error(
			`수업활동일지 정보를 불러올 수 없습니다. 로그인 상태를 확인해주세요.`,
		);
...
export async function getActivities(
	courseId: ID,
): Promise<FetchableMap<Activity>> {
	const document = cheerio.load(
		(await doorAxios.get(`/LMS/LectureRoom/CourseOutputs/${courseId}`))
			.data,
	);
	...
	if (!table)
		throw new Error(
			`수업활동일지 목록을 불러올 수 없습니다. 로그인 상태를 확인해주세요.`,
		);

	const activities = parseTableElement(table).map(activity => {
		const from = moment(
			activity['제출기간'].text.split('~')[0].trim(),
			'YY-MM-DD HH:mm',
		).toDate();
		const to = moment(
			activity['제출기간'].text.split('~')[1].trim(),
			'YY-MM-DD HH:mm',
		).toDate();
	...
	return {
		...
		items: Object.fromEntries(
			activities.map(activity => [activity.id, activity]).reverse(),
		),
