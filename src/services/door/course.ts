...
export async function getCourses(): Promise<FetchableMap<Course>> {
	...
	const courseTable = document(
		`#wrap > div.subpageCon > div:nth-child(3) > div:nth-child(3) > table`,
	)
		.toArray()
		.shift();
	...
	if (!courseTable)
		throw new Error(
			'강의 목록을 불러올 수 없습니다. 로그인 상태를 확인해주세요.',
		);
...
export async function getCourseDetail(id: ID): Promise<Course> {
	const document = cheerio.load(
		(await doorAxios.get(`/LMS/LectureRoom/CourseLetureDetail/${id}`)).data,
	);

	// 수업계획 (수업계획서)
	const descriptionTable = document(
		`#wrap > div.subpageCon > div:nth-child(5) > div:nth-child(3) > table`,
	)
		.toArray()
		.shift();
	...
	const ratesTable = document(
		`#wrap > div.subpageCon > div:nth-child(5) > div:nth-child(5) > table:nth-child(1)`,
	)
		.toArray()
		.shift();
	...
	if (!descriptionTable || !ratesTable || !scheduleTable)
		throw new Error(
			'강의 정보를 불러올 수 없습니다. 로그인 상태를 확인해주세요.',
		);

	const description = parseInformaticTableElement(descriptionTable);

	const rates = parseTableElement(ratesTable).find(
		row => row['평가항목'].text === '비율',
	);
