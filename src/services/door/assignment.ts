...
export async function getAssignment(courseId: ID, id: ID): Promise<Assignment> {
	...
	const descriptionTable = document(
		`#sub_content2 > div:nth-child(1) > table`,
	)
		.toArray()
		.shift();
	const resultTable = document(
		`#sub_content2 > div:nth-child(3) > table:not(.tbl_type)`,
	)
		.toArray()
		.shift();
	const submissionTable = document(`#sub_content2 > div.form_table_s > table`)
		.toArray()
		.shift();
	const form = document(`#CourseLeture`).toArray().shift();

	if (!descriptionTable || !submissionTable || !form)
		throw new Error(
			`과제 정보를 불러올 수 없습니다. 로그인 상태를 확인해주세요.`,
		);
	...
	const result = resultTable
		? parseInformaticTableElement(resultTable)
		: undefined;

	const attachments: Attachment[] = [];

	document('a', description['첨부파일'].element)
		.toArray()
		.forEach(file => {
	...
	const resultScore =
		Number(result?.['점수']?.text?.match(/\d+/)?.[0]) || undefined;

	const from = moment(
		description['제출기간'].text.split('~')[0].trim(),
		'YY-MM-DD HH:mm',
	).toDate();
	const to = moment(
		description['제출기간'].text.split('~')[1].trim(),
		'YY-MM-DD HH:mm',
	).toDate();
	...
	const bonusPeriod =
		description['추가 제출기간'].text.trim() === '없음'
			...
			: {
					from: moment(
						description['추가 제출기간'].text.split('~')[0].trim(),
						'YY-MM-DD HH:mm',
					).toDate(),
					to: moment(
						description['추가 제출기간'].text.split('~')[1].trim(),
						'YY-MM-DD HH:mm',
					).toDate(),
			  };
...
export async function getAssignments(
	courseId: ID,
): Promise<FetchableMap<Assignment>> {
	const document = cheerio.load(
		(
			await doorAxios.get(
				`/LMS/LectureRoom/CourseHomeworkStudentList/${courseId}`,
			)
		).data,
	);
	...
	if (!table)
		throw new Error(
			`Cannot fetch Assignment List. Please check login status.`,
		);

	const assignments = parseTableElement(table).map(assignment => {
		const from = moment(
			assignment['제출기간'].text.split('~')[0].trim(),
			'YY-MM-DD HH:mm',
		).toDate();
		const to = moment(
			assignment['제출기간'].text.split('~')[1].trim(),
			'YY-MM-DD HH:mm',
		).toDate();
