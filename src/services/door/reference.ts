...
export async function getReference(courseId: ID, id: ID): Promise<Reference> {
	...
	const document = cheerio.load(
		(await doorAxios.get(`/BBS/Board/Read/CourseReference/${id}`)).data,
	);
	...
	if (!detailTable)
		throw new Error(
			'강의자료를 불러올 수 없습니다. 로그인 상태를 확인해주세요.',
		);
...
export async function getReferences(
	courseId: ID,
): Promise<FetchableMap<Reference>> {
	const document = cheerio.load(
		(
			await doorAxios.get(
				`/BBS/Board/List/CourseReference?cNo=${courseId}&pageRowSize=200`,
			)
		).data,
	);
	...
	if (!table)
		throw new Error(
			'강의자료 목록을 불러올 수 없습니다. 로그인 상태를 확인해주세요.',
		);
