...
export async function getTeamProject(
	...
	const descriptionTable = document(
		`#sub_content2 > div.form_table_b > table`,
	)
		.toArray()
		.shift();
	const submissionTable = document(`#CourseLeture > div.form_table_s > table`)
		.toArray()
		.shift();
	...
	if (!descriptionTable || !submissionTable || !form)
		throw new Error(
			`팀 프로젝트 정보를 불러올 수 없습니다. 로그인 상태를 확인해주세요.`,
		);
...
export async function getTeamProjects(
	courseId: ID,
): Promise<FetchableMap<TeamProject>> {
	const document = cheerio.load(
		(
			await doorAxios.get(
				`/LMS/LectureRoom/CourseTeamProjectStudentList/${courseId}`,
			)
		).data,
	);

	const table = document(`#sub_content2 > div:nth-child(4) > table`)
		.toArray()
		.shift();

	if (!table)
		throw new Error(
			`팀 프로젝트 목록을 불러올 수 없습니다. 로그인 상태를 확인해주세요.`,
		);

	const teamProjects = parseTableElement(table).map(teamProject => {
		const from = moment(
			teamProject['제출기간'].text.split('~')[0].trim(),
			'YY-MM-DD HH:mm',
		).toDate();
		const to = moment(
			teamProject['제출기간'].text.split('~')[1].trim(),
			'YY-MM-DD HH:mm',
		).toDate();

		return {
			id: teamProject['팀프로젝트 제목'].url?.match(
				/ProjectNo=(\d+)/,
			)?.[1],
			courseId: courseId,
	...
	return {
		...
		items: Object.fromEntries(
			teamProjects
				.map(teamProject => [teamProject.id, teamProject])
				.reverse(),
		),
